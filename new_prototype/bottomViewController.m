...
CGFloat LOW = 0.3 ;
int state=0;
CGFloat quality_number = 1.0;

@interface bottomViewController ()

@property (weak, nonatomic) NSString *tQuality ;
...
{
    if(state==1){
        self.high.backgroundColor=[UIColor whiteColor];
    ...
    if(state==2){
        self.medium.backgroundColor=[UIColor whiteColor];
    ...
    if(state==3){
        self.low.backgroundColor=[UIColor whiteColor];
...
-(void) uploadData{
   ...
   /* NSString *FileParamConstant5 = @"book[pages_attributes][1411060113747][image]";
    ...
    NSData *imageData = UIImageJPEGRepresentation(test_image, quality_number);
    ...
    NSData *imageData2 = UIImageJPEGRepresentation(test_image2, quality_number);
    ...
    NSData *imageData3 = UIImageJPEGRepresentation(test_image3, quality_number);
    ...
    NSData *imageData4 = UIImageJPEGRepresentation(test_image4, quality_number);
    ...
    */
    ...
    NSString *search = @"/ebook/";
    NSRange range = [str rangeOfString:search];
    
    NSLog(@"位置：%d || 字串相同長度：%d", range.location, range.length);
    self.number = [str substringWithRange:NSMakeRange(range.location+range.length, 3)];
    NSLog(@"NUMBER:%@",self.number );
    
    //[self performSegueWithIdentifier:@"TenToFinal" sender:self];
}
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    if([segue.identifier isEqualToString:@"TenToFinal"]){
        ...
        [self uploadData];
        NSLog(@"after upload number: %@",self.number);
        self.data.final_number=self.number;
        NSLog(@"after final: %@",self.data.final_number);

        browserViewController *vc10_f = segue.destinationViewController;
        vc10_f.data = self.data;
         NSLog(@"after vc:%@",vc10_f.data.final_number);
    }
}
